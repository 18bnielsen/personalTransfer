...
* Modify main.c to change the values here.
  ...
  <mark>[Period = 10000/sample Rate]</mark>
...
* Why did the “Period” on the timer need to be slightly adjusted from your calculation?  Why didn’t the exact number just work?
  ...
  <mark>[There are hardware constraints that require slight adjustments. Also that in calcuating and dividing, some rounding errors can ocur. Aditionally, the value may not be able to be done by a specific timer with certain resolutions or abilities. ]</mark>

* In the current approach, could the serial port Baud rate affect the ADC Sample Rate?
  
  <mark>[Yes, because the ADC values are transmitted via the serial interface. Meaning there are multiple different values that want to use the serial bus. If the baud rate is low, this means the serial port will talk longer, clogging up the bus for longer, potentially delaying the ADC Sample]</mark>
...
* How could you change the code/structure of the ADC sampling, currently controlled by the timer ISR, to make it require less empirical adjustment?
  ...
  <mark>[You could add an interupt to a timer that is set to the desired sample rate]</mark>
